...
c	CREHDYS v2 : corrections were made compared to original  CREHDYS model (flow convergence, erosion calculation, theta adjustment, settling velocity) 
...
c	Developed by Eric Laloy and Charles Bielders, revised by Timothée Clement
...
c	Block 3 - Variables for rainfall, rainfall kinetic energy and climat
	...
	real rcc(90000), rtc(90000), dtm,tc,
     .	 rc,rain,pinst,tempc,soitemp,radi
...
c	Block 8 - Runoff dynamics and balance
     ...
     .	QIDT,lat,QOUTTEST,QMMHR, TOTQMMHR, TOTQOUTLET, chwid,mnch,mn,QOUTLET,WEIGHTR
...
c	Block 9 - Crop rotation and cover, LAI and biomass development
	...
	real kextint, kextmaize, gcoverint, kcovermaize, biomcover,
     .	rbiomcover,KCCOVER,KEXT,prstlai,lstbiomcover,FN,FW,KDEG, prstlaimaize, prstlaiint

	real sumtemp, sumtempdeg, checkd,PRSTCOVER, prstcoverint, prstcovermaize
...
c	Block 11 - Erosion processes
	...
	real qs22(100485), qs21(100485),qs12(100485),qs11(100485),
     ...
     .	VS,d50,CTC,DTC,COH,YDET,TOTEROSTOT, TOTEROS, CCOUTLET, TOTQSOUTLETS,
...
c	read input data
      read(1,*) plotname
	read (1,*) simdur
      read (1,*) dt		! read(unit number identifier=1='gen.inp', format identifier=1=identifier du format défini à la ligne juste ci-dessous, en l'occurrence le 1). suivi du nom des variables auxquelles assigner les valeurs lues.
...
c		read(13,*) rr(i)
	...
	do i=1,n
c		read(16,*) mann(i)
		if (wheel(i).eq.0) mann(i)=man2		! manning homogène spatialement. Ce manning n'est plus changé et est bien utilisé.
...
c		read(17,*) cover(i)
	...
	do i=1,n
		read(18,*) theta(i)		! initial soil moisture content [m³ water/m³soil] for each element, peut être hétérogène spatialement
      end do
...
!	close(9)
	...
	neff=0
	df(1)=df(1)*10 ! convrrt df [cm] to df [mm]
	do i=1,n		! loop on all grid elements (cells)
	...
	tcst=dt/3600	! fraction of dt [s] in 1 hour = coefficient to convert rain [mmhr-1] to [mm per time step]
      ...
      do i=1,n
              ...
              moutlet=i ! moutlet is indice of (the last) outlet cell
...
c	maximum evaporative depth (mm)
	edx(i)=90.-0.77*cl(i)+0.006*sa(i)*sa(i) ! max evaporative depth [mm]
...
c	Vs = settling velocity - stokes law spherical particle Chow et al. 1988
	...
	Vs=(1./18)*((Diam)**2)*9.81*(rhop-rhow)/nudynwat ! Particle settling velocity [m/s]. Original CREHDYS : Vs=	10*(2./9)*((Diam*0.5)**2)*9.81*(rhop-rhow)/nudynwat. BUT why *10 factor at the beginning ?
...
c	erosion variables
	 ...
	 qs2t2(m)=0.	
      ...
      ! original CREHDYS : if(m.lt.90001) rcc(m)=0. ! Initialise le throughfall rate à zéro partout, mais erreur car boucle dans m (et devrait être dans lline)
	end do
...
c	read weather data
	...
	read(2,*) day,tempc,soitemp,radi,raites,starteventtime ! Comme ce statement READ(2,*) est appelé plusieurs fois (dans la subroutine rainfa lorsqu'il y a événement pluvieux, et ici L901 dans la boucle de simulation journalière), chaque fois que READ(2,*) ext exécuté, FORTRAN "retient" combien de fois il a déjà read cet input et donc va automatiquement à la ligne suivante donc on se trouve automatiquement dans la journée de simulation en cours
      lsttr=0. ! lsttr is time of the end of event [min]. if = 0, indicate that we are before the event
c	cover developement (percentage cover, biomass, LAI) routines
	...
	lstbiomcover = 0
      ...
      if (dstart.gt.0.and.idate.ge.dinterp(1).and.dinterp(1).ne.0) then ! there is a cover developping or decaying (dstart>0), idate>1st day of intercrop period, 
          ...
          elseif (destruction.gt.0.and.idate.ge.(dinterp(1)+destruction).and.
     &          (burial.eq.0.or.idate.lt.(dinterp(1)+destruction+burial))) then ! after destruction and before burial (or no burial) : decay
     ...
     .        (95+1*(exp(kcovermaize*sumtemp)-1))) ! cover=f(sumtemp)
              if (kextmaize.gt.0) prstlaimaize= -log(1-prstcovermaize)/kextmaize
      else
          prstcovermaize=0
          prstlaimaize=0 
      end if
      ...
      write(381,3772) idate,curcropper,tempc,biomcover,(100*prstcoverint),(100*prstcovermaize),prstlaiint,prstlaimaize ! write crop outputs
3772  format (i5,1x,A10,1x,f6.1,1x,f10.2,1x,f10.1,1x,f10.1,1x,f10.2,1x,f10.2) ! format to write crop outputs
...
c maize stem and canopy flow
	...
	if (prstmaize.gt.0) then ! maize period
		...
		Trf= rcc(i)*tcst*(prstcovermaize)*brandt*0.5  ! Trf=Kinetic energy content of drops falling off maize leaves 
              
	else ! not maize period -> intercrop period or 
...
c	compute runoff 150 - 15
	...
	do 150 m2=0,maxval(flacc) ! loop on the flow accumulations
		...
		do 15 m=1,n		!avant : n2 ! spatial loop on all cells of the grid. This double loop, with the if statement at L1150, enables to perform a do loop on all cells in the flow acc order
...
c	test for runoff
	...
	!		
     ...
     .				' at idate = ',idate
					read(*,*) ! some input from user (e.g. enter "enter") is needed to continue
...
c	 to avoid to low outflow
	...
	if (flacc(m).eq.maxval(flacc)) then ! one of outlets (and there is runoff because at this point of the code
...
c	***** end of Kinematic wave *****
 ...
 3766 format(i5,1x,f7.2,1x,f7.2,1x,f10.4) ! format not used. Rem : these formats are created each cell of each computation time step (which is not needed?)
 3769 format(i5,1x,f7.2,1x,f7.2,1x,f7.2,1x,f10.4,1x,f10.4,1x,f10.4,1x,f10.4,1x,f15.10,1x,f15.6,1x,f7.4) ! format to write runoff outputs
...
c	here you can print the water balance and erosion information associated with pixel m within the event : theta(m), qp22(m) m3/s, qs22(m) kg/s,...
  ...
  150 continue ! end runoff loop ; flow acc loop on the number of cells to the outlet
	...
	write(380,3771) idate,lstr,(timnum2-1)*(dt/60.), ! write event erosion outputs
     .           	   totqsoutlets, Transc, totqsoutlets/totqoutlet, Dep, Dfl, 
     .				Dsplash,
     .				eros
 3771 format(i5,1x,f7.2,1x,f7.2,1x,f10.4,1x,f15.10,1x,f15.3,1x,      ! format to write erosion outputs
...
c	evaporation, transpiration & percolation routine
	...
	edisp =	(theta(i))*df(i)			!Available water in the control volume ; [mm]

	edispm = min((theta(i))*edx(i),edisp)    ! Available water for soil evaporation = minimum between available water in the evaporative depth [mm] and in the control volume [mm]. In others words, minimum between edx and df

	edispmfc = min(fc(i)*edx(i),fc(i)*df(i))		! Available water for soil evaporation at field capacity [mm]
...
c	soil moisture not limiting,drainage
     ...
     .				dt,a(i),idate,percol,lsttr,raites,starteventtime)
		...
		! percol = percol*24. ! < original CREHDYS but percol already in [mm/day]

		totwloss = tetpr + percol ! total water loss from the control volume = total evapotranspiration + percolation [mm/day]
...
c	soil moisture limiting, no drainage
		...
		if (theta(i).lt.asmlim(i)) then ! asmlim=wp -> theta<wp
			...
			if (edisp.gt.tetpr) then
				...
				if (theta(i).lt.wp(i)) then
                      totwloss = tetpr - (wp(i)-theta(i))*df(i)
                      theta(i) = wp(i)
                  end if
			elseif (theta(i).eq.wp(i)) then	  ! never happen         
				totwloss=0.	
				es(i)=0.
				pep(i)=0.

		    else ! edisp<=tetpr AND theta<WP -> ? this condition never happen ?
				...
				tetpr = edisp
                  totwloss=tetpr ! totwloss=edisp
				theta(i) = (edisp - tetpr)/df(i)	!+ wp(i) ! Puisqu'à ce stade tetpr=edisp, cela revient à dire theta=0 (mais après cfr L1957 : theta=wp)
			
				if (theta(i).lt.wp(i)) theta(i) = wp(i) ! no end if needed because statement on same line

			end if	! end if edisp vs tetpr

		else ! theta>=asmlim -> theta >=wp (and <=FC)
	       ...
	       elseif (theta(i).eq.wp(i)) then
				...
				tetpr = edisp
                  totwloss=tetpr ! totwloss=edisp
				theta(i) = (edisp - tetpr)/df(i)	!+ wp(i) ! Puisqu'à ce stade tetpr=edisp, cela revient à ramener theta à 0 donc à WP à L1986
...
c	here print the runoff and erosion information associated with pixel of interest after the event
	...
	write(375,3777) idate,totprec,totrun,(toteros/(neff*dx2))*10,thetaoutlet ! write daily runoff and erosion outputs in daily.out
 3777 format(i5,f8.4,f8.4,' mm',1x,f16.4,' t/ha',f16.4)
	write(378,3780) idate,totprec,totrun,(toteros/(neff*dx2))*10 ! write daily runoff and erosion outputs in dailyRE.out
 3780 format(i5,f8.4,1x,f8.4,1x,f16.4)
      write(382,3773) idate,totprec,keff(moutlet),rrv(moutlet),
     .                cuminf(moutlet),draintot(moutlet),es(moutlet),pep(moutlet),totwloss,thetaoutlet ! write soil outputs
 3773 format(i5,1x,f8.2,1x,f8.2,1x,f8.2,1x,f8.2,1x,f8.2,1x,f8.2,1x,f8.2,1x,f8.2,1x,f8.4)
...
c	 Chu, WRR 1978 
	...
	real s,r,ke, thets,theti,psimat,fmax
      real(8) fcerr,dfcerr,cinf ! real(8) enables better precision on these variables, preventing any bug of stuck in while loop of Newton-Raphson iterations (when fcerr is impossible to reach below error tolerance eps)
	...
	if (t.eq.0.or.t.eq.dtm) then ! If one of the two first time steps of event : impossible de calculer les variables pour le time step précédent ET il faut ramener les variables à zéro entre les événements successifs
	pond(m)=0.
	excess(m)=0.
	oldp(m)=0.
...
********************************
 20   continue
	...
	ti=((tetinst-fc)*df)/rkfc ! ti=travel time trough control volume [h]. ? Why *thets ?

	if (ti.lt.(dt/3600.)) ti=dt/3600. ! travel time ne peut pas être inférieur au event time step

	dr=df*(tetinst-fc)*(1-exp(-(dthr/ti))) !dr= [mm/dthr] ! Original Laloy comment : "dr in mm/h". MAIS FAUX puisque cfr Savabi & Williams dt=dthr
...
c-------------------------------------------------------------------------------------------------	
	subroutine perco(theti,ks,thets,fc,df,dt,ks2,idate,percol,
     .lsttr, raites, starteventtime) ! perco = fonction pour calculer la percolation entre les événements (time step=1jour). fonction drain est l'équivalent au sein des events
			! L1901 : call perco(theta(i),keff(i),tp(i),fc(i),df(i),dt,a(i),idate,percol,lsttr,raites, starteventtime)
	implicit real (a-z)
...
c	soil evaporation
	...
	if (edispm.ge.edispmfc.and.totprec.gt.0.) then ! new stage 1
...
c	si on �tait dans second stage, on le recommence au d�but
   ...
   31	if (es1p(i).gt.esu(i)) then		! when accumulated soil evaporation > upper limit of soil evaporation -> pass from stage 1 (moisture not limiting) to stage 2 (moisture limiting)
		...
		es(i) = max(0.,esm)		! soil evaporation = potential soil evaporation (sauf si <0 (Na) alors ramené à 0) ! original CREHDYS : max1 and not max
		es2p(i) = 0.
		ttime(i) = 0
   ...
   32 espot = max(0.,esu(i))		! stage 2 (moisture limiting)
	...
	es(i) = min(esm,es2)		! Soil evaporation = minimum between potential soil evaporation when moisture is not limiting and potential soil evaporation when moisture is limiting. Logiquement, ce sera plus souvent cette dernière. MAIS ainsi, si le LAI ou la radiation ou la transmissivité sont tellement faibles qu'il est impossible d'atteindre la relation empirique du stage 2, alors on garde l'équation du stage 1
