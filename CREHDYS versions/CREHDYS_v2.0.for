...
c	Block 4 - Infiltration and drainage processes
	...
	real FIL,VERTFLUX,FILTREAL, TOTFILT, MEANFILT
...
!	close(9)
	...
	neff=neff+1	
	thetainst(i)=theta(i)   ! retrieve soil moisture values after daily calculation (evapotranspiration and percolation)
...
c	cover developement (percentage cover, biomass, LAI) routines
	...
	if (kccover.gt.0) then ! there is a cover developing (maize or intercrop)
     ...
     .		   (95+1*(exp(kccover*sumtemp)-1)))		    ! cover=f(sumtemp)
		...
		if (idate.lt.(dstart+destruction)) then ! before destruction
			...
			biomcover =	kccover*(sumtemp**2)	! cover crop dry biomass [g/m²] as function of sumtemp [°C days]) - expon. curve
		...
		elseif (burial.gt.0) then ! there is a destruction and a burial
			...
			if (idate.lt.(dstart+destruction+burial)) then	! degradation (before burial)
			...
			biomcover =	exp(fN*fW*kdeg*sumtempdeg)*lstbiomcover ! douglas & rickman cover crop decay equation

			prstcover =  0.004*biomcover ! prstcover = % cover by residues
			if (prstcover.gt.0.95) prstcover=0.95
	...
	else ! kcover=0 : there is no cover developing (were are neither in intercrop nor maize period)
	...
	prstlai=0
      ...
      write(381,3772) idate,curcropper,tempc,(100*prstcover),biomcover,prstlai ! write crop outputs
3772  format (i5,1x,A10,1x,f4.1,1x,f5.2,1x,f6.1,1x,f5.2) ! format to write crop outputs
...
c	update thetainst after infiltration and drainage
	...
	draintot(m) = draintot(m) + dr(m)*dt/3600  ! cumulative drainage during the rainfall event [mm]
...
c	soil moisture not limiting,drainage
		...
		percol = 0.
          if (raites.eq.0) then ! do percolation calculation only if there was no rain event (in this case indeed drainage calculation not already (partly) done)
...
c	soil moisture limiting, no drainage
	...
	if (raites.eq.0) draintot(i)=percol ! record total daily percolation (if percolation was not calculated during event)
...
c	here print the runoff and erosion information associated with pixel of interest after the event
 ...
 3780 format(i5,f8.4,1x,f8.4,1x,f16.4)
      write(382,3773) idate,totprec,keff(moutlet),cuminf(moutlet),draintot(moutlet),es(moutlet),pep(moutlet),totwloss,thetaoutlet ! write soil outputs
 3773 format(i5,1x,f5.2,1x,f6.2,1x,f5.2,1x,f5.2,1x,f5.2,1x,f5.2,1x,f5.2,1x,f6.4)
...
********************************
	...
	percol=df*((theti)-fc)*(1-exp(-(dthr/ti))) ! [mm/jour] because dthr is=24h. original CREHDYS : [mm/h]
