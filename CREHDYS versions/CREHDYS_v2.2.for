...
c
	real dxvert, dxdiag, dx2, cu, tcst, t0, t1 ! toutes les variables doivent être prédéfinies dans ces BLOCKS avant d'être changées par leur valeur en input 
	real dxf
	real dstart
	real dt, dx
...
c	Block 2 - Hill-slope characteristics, including flow routing informations
	...
	integer nsoil,i,j,ncrop,m, ! nsoil and ncrops = number of soils and crops. i,j=indices de toutes sortes, à utiliser dans les boucles etc. n=total number of grid elements (cells)=nrow*ncol. m=indice de la cellule (c-à-d de l'élément de grid) dans le fichier input (1 dimension)
     . x, y, count
...
c	Block 4 - Infiltration and drainage processes
	...
	real thetainst(100485),theta(100485),psi(100485), thetaoutlet,
     .	ke(100485),pond(100485), asmlim(100485), lsttr ! because the dimension of cuminf is declared in the common block (L89), the size of the array should not be specified here
...
c	Block 5 - Evapotranspiration process
	real albed(100485) ! because the dimension of these variables are declared in the common block (L89), the size of the arrays should not be specified here
...
c	corresponding retention
	  ...
	  if (rrksovar.eq.1) then ! If Ksat and RR are varying during the season
	    dirvon(i)=(0.294*rrcm+0.031*rrcm*rrcm-0.012*rrcm*sl(i)*100)*10.0   ! Surface retention [mm]	
...
c             Compute total water and sediment inflow in current cell (=sum of outflows from surrounding cells)
	...
	s(m) = ss(m) + (rcc(i) - filtreal)*dt/3600 + hindt ! original CREHDYS : + (rcc(i) - filt(m))*cu*1000*dt/dx2 (complicated BUT also filt(m) while filtreal must be used). adjust total ponding water of the cell [mm] at that computation time step = ss (previous s) + rain - infiltration + incoming runoff
...
c *******	no runoff *******
	...
	ss(m)=(rcc(i) - filtreal)*dt/3600 + ss(m) ! adjust previous total ponding water of the cell [mm]
...
c	goto 99
	...
	Dsplash = Lerosionsplash(dt,dx2,hmm, ekbel,ekbelcovsplash,rc(i),  ! splash detachment on (covered and uncovered fraction of) soil [kg/s] (eq. (2) and (3) from Laloy & Bielders 2009a)
     . rcc(i), As, prstcover) !kg/s                                 ! original CREHDYS : ekbelcov2 was used instead of ekbelcovsplash !
...
! Lisem model:	.and.q2t2(m).gt.0
	if (q2t1(m).gt.0) then ! if not this if statement, NaN problems because null denominators
	   qs22(m)=(ebar*dx
...
c------------------------------------------------------------------------------
c------------------------------------------------------------------------------
	real function rain(rate,prstlai,i,dt,t,idate,cu,prstcover) ! Calculate net rainfall = throughfall rate (after interception)	! Contrairement à une subroutine, une function must return a single value, and can be invoked from within expressions, like a write statement, inside an if declaration if (function) then , etc
...
c------------------------------------------------------------------------------------------------	
c------------------------------------------------------------------------------------------------	       
	real function drain(tetinst,ks,thets,fc,df,dt,ks2,idate) ! calcul de percolation au time step de l'événement (dt). Pour time step journalier (entre event), voir subroutine "perco"
...
c------------------------------------------------------------------------------------------
c------------------------------------------------------------------------------------------
	real function Lerosionsplash(dt,dx2,hsvmm, ek,ekt,p,pt, ! L1718 : Dsplash=Lerosionsplash(dt,dx2,hmm, ekbel,ekbelcovsplash,rc(i),rcc(i),As,prstcover)
     .                         As, prstcover)           ! original CREHDYS : ekbelcov2 was unsed instead of ekbelcovsplash !
c	implicit real (a-z)
	real dt, dx2, hsvmm, ek, ekt, p, pt, As, prstcover
