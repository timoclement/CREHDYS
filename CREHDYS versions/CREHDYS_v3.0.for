...
c	Ds_r = rainfall (uncovered fraction), Ds_t = troughfall (vegetation-covered fraction)	
	...
	Ds_r_ponded=1e-3*((2.82/As)*ek*exp(-1.48*hsvmm)+2.96)*p*Ar_ponded/3600	![kg/s] (eq.(2) from Laloy & Bielders, 2009a) ! ponded
...
c	Ds_r=1e-3*((2.82/As)*ek*dexp(-1.48*hsvmm)+2.96)*(p*dt/3600)*Ar/dt	!kg/s
	...
	Ds_t_ponded=1e-3*((2.82/As)*ekt*exp(-1.48*hsvmm)+2.96)*pt*At_ponded/3600	!kg/s
